...
import { Star, Bookmark, BookmarkCheck, Eye, TrendingUp, Mail, Phone } from "lucide-react"
      ...
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div
              className={`relative w-12 h-12 rounded-full ${getAvatarColor(employee.firstName)} flex items-center justify-center`}
            >
          ...
          <Button
            ...
            className={`p-1 hover:bg-gray-100 ${isBookmarked(employee.id) ? "text-yellow-500" : "text-gray-400"}`}
          >
            {isBookmarked(employee.id) ? <BookmarkCheck className="h-4 w-4" /> : <Bookmark className="h-4 w-4" />}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
          <div>
            ...
            <p className="font-semibold text-green-600">${generateSalary().toLocaleString()}</p>
          </div>
          <div>
